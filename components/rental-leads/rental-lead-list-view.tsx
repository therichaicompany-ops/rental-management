...
export function RentalLeadListView({
  ...
  const getStageLabel = (status: string): string => {
    const map: Record<string, string> = {
      new: t.rentalLeads.stages.new,
      surveying: t.rentalLeads.stages.surveying,
      negotiating: t.rentalLeads.stages.negotiating,
      agreed: t.rentalLeads.stages.agreed,
      rejected: t.rentalLeads.stages.rejected,
      converted: t.rentalLeads.stages.converted,
    }
    return map[status] ?? (LEAD_STATUS_LABELS[status as LeadStatus] || status)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t.rentalLeads.title}</h1>
          <p className="text-sm text-slate-500 mt-1">
            {t.rentalLeads.subtitle}
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Input
            type="text"
            placeholder={`${t.common.search} (${t.rentalLeads.leadCode}, ${t.locations.title})...`}
            value={searchTerm}
          ...
          >
            <option value="all">{t.common.all} ({leads.length})</option>
            {Object.entries(LEAD_STATUS_LABELS).map(([val]) => (
          ...
          <select
            ...
            className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-primary-500"
          >
            <option value="all">{t.common.all} ({t.auth.user})</option>
            {staffProfiles.map((p) => (
          ...
          <select
            ...
            className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-primary-500"
          >
            <option value="all">{t.locations.province} ({provinces.length})</option>
            {provinces.map((prov) => (
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 pt-1 border-t border-slate-100">
          <span>
            {t.common.total} {filteredLeads.length} / {leads.length} {t.common.items}
          </span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="border-b border-slate-200 bg-slate-50/80 text-xs font-semibold text-slate-700 uppercase tracking-wider">
                <tr>
                  <th className="px-4 py-3.5">{t.rentalLeads.leadCode}</th>
                  <th className="px-4 py-3.5">{t.rentalLeads.title}</th>
                  <th className="px-4 py-3.5">{t.locations.title}</th>
                  <th className="px-4 py-3.5">{t.customers.title}</th>
                  <th className="px-4 py-3.5 text-right">{t.rentalLeads.proposedRent}</th>
                  <th className="px-4 py-3.5">{t.rentalLeads.status}</th>
                  <th className="px-4 py-3.5">{t.calendar.events.negotiationFollowUp}</th>
                  <th className="px-4 py-3.5">{t.auth.user}</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-right font-mono font-medium text-slate-900 whitespace-nowrap">
                        ...
                        }).format(Number(lead.proposed_monthly_rent || 0))}
                      </td>
